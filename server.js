...
// Fetch all active registered profile nodes
...
app.put('/api/users/:id', verifyAdminAccess, async (req, res) => {
    try {
        ...
        if (password) {
            ...
            query = `UPDATE users SET name=$1,email=$2,role=$3,phone=$4,password=$5 WHERE id=$6 RETURNING id,name,email,role,phone`;
            values = [name, email, role, phone, hashedPassword, req.params.id];
        } else {
            query = `UPDATE users SET name=$1,email=$2,role=$3,phone=$4 WHERE id=$5 RETURNING id,name,email,role,phone`;
...
// Admin endpoint to implement structural soft-deletion
...
app.put('/api/case-studies/:id', verifyAdminAccess, async (req, res) => {
    try {
        ...
        const updated = await pool.query(
            `UPDATE case_studies SET title=$1, category=$2, challenge=$3, solution=$4, result=$5, image_url=$6 WHERE id=$7 RETURNING *`,
...
// Update Testimonial Approval Status & Feature Toggle
...
const updateBlogPost = async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `UPDATE blog_posts SET title=$1, category=$2, content=$3, summary=$4, image_url=$5, slug=$6, seo_title=$7, seo_description=$8 WHERE id=$9 RETURNING *`,
...
app.delete('/api/blog/:id', verifyAdminAccess, async (req, res) => {
    try {
        await pool.query('UPDATE blog_posts SET is_deleted = TRUE WHERE id = $1', [req.params.id]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/subscribers', async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `INSERT INTO subscribers (email, status) VALUES ($1, 'active')
...
app.delete('/api/subscribers/:id', verifyAdminAccess, async (req, res) => {
    try {
        await pool.query('UPDATE subscribers SET is_deleted = TRUE WHERE id = $1', [req.params.id]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.delete('/api/consultations/:id', verifyAdminAccess, async (req, res) => {
    try {
        await pool.query('UPDATE consultations SET is_deleted = TRUE WHERE id = $1', [req.params.id]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
